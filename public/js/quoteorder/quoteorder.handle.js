        ...
        var actions = {
                ...
                return new Promise(function (resolve, reject) {
                    try {
                        ...
                        tempArray = tempArray.map(function (item) {
                            return item['패키지'].indexOf(',') >= 0 ? item['패키지'].split(',') : item['패키지']
                        })

                        $.each(tempArray, function (i, el) {
                            if (typeof el !== 'string') {
                                $.each(el, function (_i, _el) {
                                    if ($.inArray(_el, packages) === -1) packages.push(_el);
                                })
                            } else {
            ...
            renderProductList: function () {
                ...
                if (total > 0) {
                    $TARGET.append(
                        '<tr>' +
                        '    <td colspan="7" class="text-right bg-navy">' + total.toLocaleString() + '</td>' +
                        '    <td class="bg-navy"></td>' +
                        '</tr>'
                    )
                }
            ...
            saveOrder: function (data) {
                axios.post(API.QUOTEORDER.ORDER, data)
                    .then(function (result) {
                        swal('견적/발주 등록', '저장되었습니다.', 'success')
                            .then(function () {
                                location.href = '/quoteorder';
                            })
                    })
                    .catch(function (error) {
                        swal('견적/발주 등록', '오류가 발생하였습니다.', 'error')
                            .then(function () {
                                location.href = '/quoteorder';
                            })
                        fn.errorNotify(error)
