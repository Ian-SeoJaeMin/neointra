(function (exports) {
    ...
    var $VISITADD = $('#visit-save');
    var $VISITMODIFY = $('#visit-modify');
    ...
    $VISITUNREG.bind('ifChecked ifUnchecked', function (event) {
        $VISITUNREGHOSPITAL.toggleClass('hidden');
        if (event.type === 'ifChecked') {
            $VISITHOSPITAL.selectpicker('hide');
            data.USER_ID = '-1';
        } else {
            $VISITHOSPITAL.selectpicker('show');
            data.USER_ID = '';
        }
    });
    ...
    */
    $VISITSTARTDATE.val(data.start.date)
        .bind('dp.change', function (event) {
            data.start.date = event.date.format('YYYY-MM-DD');
            $VISITENDDATE.data('DateTimePicker').minDate(event.date);
        });
    $VISITENDDATE.val(data.end.date)
        .bind('dp.change', function (event) {
            data.end.date = event.date.format('YYYY-MM-DD');
    ...
    $(document).ready(function () {
        $('[data-id="visit-hospital"]').trigger('click').find('input').focus();;
    });
